...
- Dependency scan: !`npm audit --audit-level=moderate 2>/dev/null || echo "No package manager detected"`
...
3. **Authentication & Authorization**
   ...
   - Examine password/secret storage (ADMIN_PASSWORD, SUPABASE_SERVICE_ROLE_KEY)

4. **Input Validation & Sanitization**
   - Check all user input validation and sanitization
   - Look for SQL injection vulnerabilities
   - Identify potential XSS (Cross-Site Scripting) issues
   - Review file upload security and validation (BI/selfie verification photos)

5. **Data Protection**
   - Identify sensitive data handling practices (BI numbers, photos, deposit proofs)
...
8. **Infrastructure Security**
   - Review Vercel deployment and edge middleware (proxy.ts) security
   - Check CI/CD pipeline security
   - Examine Supabase project configuration and permissions
